...
#define SPEEDUP_TIME 15 // czas po ktÛrym nastÍpuje przyúpieszenie w s
#define SPEEDUP_FACTOR 20 // wspo≥czynnik o jaki przyúpiesza gra w s
#define GAME_SPEED 100 // poczπtkowa prÍdkoúÊ gry
...
int MainLoop(GameContext* ctx, Snake* snake, Dot* dot, BonusDot* bonus) {
	...
	while (running) {
		...
		if (!gameOver) {
			/*DrawGrid(ctx);*/ // siatka po ktÛrej porusza siÍ wπø
